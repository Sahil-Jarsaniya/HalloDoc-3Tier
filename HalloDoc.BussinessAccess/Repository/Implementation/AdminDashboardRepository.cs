    ...
    public class AdminDashboardRepository : IAdminDashboardRepository
    {
        ...
        public Chat ChatWithProvider(int reqCLientId, int adminId)
        {
            ...
            var phyHistory = (from t1 in _db.PhysicianChats
                              join t2 in _db.Physicians on t1.PhysicianId equals t2.Physicianid
                              where t1.ReqClientId == reqCLientId && t1.PhysicianId == phyId.Physicianid && t1.SenderAccountType == (int)enumsFile.AccountType.Admin
                              select new ChatHistory()
                              {
                                  Message = t1.Message,
                                  CreatedAt = TimeOnly.FromDateTime(t1.CreateTime),
            ...
            var adminRow = _db.Admins.FirstOrDefault(x => x.Adminid == adminId);
            var data = new Chat()
            {
                Sender = adminRow.Firstname + " " + adminRow.Lastname,
                SenderId = adminId,
                ReceiverId = phyId.Physicianid ?? 0,
                reqClientId = reqCLientId,
        ...
        public Chat ChatWithPatient(int reqCLientId, int adminId)
        {
            var history = (from t1 in _db.AdminChats
                           join t2 in _db.Admins on t1.AdminId equals t2.Adminid
                           where t1.ReqClientId == reqCLientId && t1.SenderAccountType == (int)enumsFile.AccountType.Patient
                           select new ChatHistory()
                           {
                               Message = t1.Message,
                               CreatedAt = TimeOnly.FromDateTime(t1.CreateTime),
            ...
            var patientHistory = from t1 in _db.PatientChats
                                 ...
                                 where t1.ReqClientId == reqCLientId && t1.SenderAccountType == (int)enumsFile.AccountType.Admin
                                 select new ChatHistory()
                                 {
                                     Message = t1.Message,
                                     CreatedAt = TimeOnly.FromDateTime(t1.CreateTime),
            ...
            var data = new Chat()
            {
                Sender = adminRow.Firstname + " " + adminRow.Lastname,
                SenderId = adminId,
                ReceiverId = reqRow.Userid ?? 0,
                reqClientId = reqCLientId,
                chatHistories = list.ToList(),
                Receiver = reqClientRow.Firstname + " " + reqClientRow.Lastname,
                AccountTypeOfSender = (int)enumsFile.AccountType.Admin,
        ...
        public void StoreChat(int reqClientId, int adminId, string message, int AccountTypeOfReceiver)
        {
